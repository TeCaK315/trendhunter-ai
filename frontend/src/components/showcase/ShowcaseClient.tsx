...
export default function ShowcaseClient({ initialTrends, lastUpdated: initialLastUpdated }: ShowcaseClientProps) {
  ...
  // Fetch trends on mount (SSR may pass empty initialTrends)
  useEffect(() => {
    fetchTrends();
  }, [fetchTrends]);
  ...
  useEffect(() => {
    const refreshInterval = setInterval(fetchTrends, 5 * 60 * 1000);
  ...
  const generateNewTrends = async (category?: string) => {
    ...
    setGenerateDropdownOpen(false);
    const selectedCat = category || generateCategory;
    try {
      ...
      // Always refresh trends after scan — even if success:false,
      // some trends may have been saved before dedup check
      await fetchTrends();
    } catch (error) {
    ...
    <div className="min-h-screen pb-20 lg:pb-0">
      <div className="min-h-screen">
                  ...
                  <div className="flex">
                    ...
                    >
                      ...
                      <span className="hidden md:inline">
                        {generating
                          ? `${generatingElapsed < 30 ? 'Поиск трендов' : generatingElapsed < 90 ? 'Анализ данных' : 'Почти готово'}... ${Math.floor(generatingElapsed / 60)}:${(generatingElapsed % 60).toString().padStart(2, '0')}`
                          : t.home.newIdeas}
                      </span>
                    </button>
                    <button
        ...
        <main className="px-4 sm:px-6 py-6 sm:py-8">
          ...
          {/* Login prompt for unauthenticated — show below trends, not instead of */}
          {limitMounted && !isAuthenticated && (
            <div className="mt-12 mb-8 flex flex-col items-center text-center py-10 border border-zinc-800/50 rounded-2xl bg-zinc-900/30">
              <div className="w-16 h-16 bg-gradient-to-br from-indigo-500/20 to-purple-600/20 rounded-full flex items-center justify-center mb-6 border border-indigo-500/30">
                <svg className="w-8 h-8 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                </svg>
              </div>
              <h3 className="text-lg font-semibold text-white mb-2">{t.auth.loginRequired}</h3>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 bg-white hover:bg-zinc-100 text-zinc-900 rounded-xl font-medium transition-all text-sm"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-white rounded-xl font-medium transition-all border border-zinc-700 text-sm"
                >
