...
export default function generate(ctx: BlockContext): BlockResult {
  ...
  return {
    'src/components/Wishlist.tsx': `'use client';
...
export function WishlistButton({ item, size = 'md' }: { item: WishlistItem; size?: 'sm' | 'md' }) {
  ...
  return (
    <button
      ...
      style={{ color: active ? '#ef4444' : '${t.text50}' }}
  ...
  if (count === 0) {
    return (
      <div className="text-center py-12">
        <Heart className="w-12 h-12 mx-auto mb-3" style={{ color: '${t.text50}' }} />
        <p className="text-sm" style={{ color: '${t.text50}' }}>Список избранного пуст</p>
        <p className="text-xs mt-1" style={{ color: '${t.text50}' }}>Нажмите ♡ на любом товаре</p>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold flex items-center gap-2" style={{ color: '${t.text}' }}>
          ...
          <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: '${t.primary10}', color: '${t.primary}' }}>{count}</span>
        </h3>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {items.map(item => (
          <div key={item.id} className="flex gap-3 p-3 rounded-xl border" style={{ borderColor: '${t.primary40}' }}>
            <div className="w-16 h-16 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: '${t.primary10}' }}>
              {item.image ? <img src={item.image} alt="" className="w-full h-full object-cover rounded-lg" /> : <span className="text-xl">📦</span>}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate cursor-pointer hover:underline" style={{ color: '${t.text}' }} onClick={() => onItemClick?.(item)}>
                {item.name}
              </p>
              {item.price !== undefined && (
                <p className="text-sm font-bold mt-1" style={{ color: '${t.primary}' }}>{item.price.toLocaleString()}{currency}</p>
              )}
            </div>
            <button onClick={() => removeItem(item.id)} className="p-1 rounded hover:opacity-70 self-start">
              <Trash2 className="w-4 h-4" style={{ color: '${t.text50}' }} />
            </button>
          </div>
        ))}
      </div>
