...
1. pain_match — почему эта компания может испытывать боль "${painPoint}" (коротко, 1-2 предложения). Если не уверен — напиши "Требует проверки"
2. outreach_angle — как лучше к ним обратиться (коротко, 1 предложение)
3. relevance_score — 1-10 насколько релевантна (на основе description)

ВАЖНО: НЕ выдумывай информацию о компаниях. Опирайся ТОЛЬКО на данные выше.
