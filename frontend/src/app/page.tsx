...
export default function Home() {
  ...
  const fetchTrends = async () => {
    setRefreshing(true);
    try {
      // Сначала восстанавливаем из кеша (мгновенно)
      if (trends.length === 0) {
        try {
          const cached = localStorage.getItem('th_trends');
          if (cached) {
      ...
      if (data.trends && data.trends.length > 0) {
        ...
        // Сохраняем в кеш
        try { localStorage.setItem('th_trends', JSON.stringify({ trends: data.trends, lastUpdated: data.lastUpdated })); } catch { /* ignore */ }
      }
    } catch (error) {
