        ...
        {!showSettings ? (
          <div className="p-5">
            <div className="flex items-start gap-3 mb-4">
              <Cookie className="w-6 h-6 flex-shrink-0 mt-0.5" style={{ color: '#6366f1' }} />
              <div>
                <h3 className="text-sm font-bold" style={{ color: '#e2e8f0' }}>Мы используем cookies</h3>
                <p className="text-xs mt-1" style={{ color: '#e2e8f070' }}>
                  Мы используем файлы cookie для улучшения работы сайта. Вы можете настроить предпочтения или принять все.{' '}
                  <a href={privacyUrl} className="underline" style={{ color: '#6366f1' }}>Политика конфиденциальности</a>
                </p>
              </div>
            </div>
          ...
          <div className="p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold" style={{ color: '#e2e8f0' }}>Настройки cookies</h3>
            ...
            <div className="space-y-3 mb-4">
              {categories.map(cat => (
                <div key={cat.key} className="flex items-center justify-between py-2 border-b" style={{ borderColor: '#6366f140' }}>
                  <div>
                    <p className="text-sm font-medium" style={{ color: '#e2e8f0' }}>{cat.label}</p>
                    <p className="text-xs" style={{ color: '#e2e8f050' }}>{cat.desc}</p>
                  </div>
                  <button
                    disabled={cat.locked}
                    ...
                    style={{ background: preferences[cat.key] ? '#6366f1' : '#6366f120' }}
                  >
