  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="flex border rounded-xl overflow-hidden" style={{ borderColor: '#6366f140' }}>
          <button onClick={() => setView('grid')} className="p-2.5" style={{ background: view === 'grid' ? '#6366f110' : 'transparent', color: view === 'grid' ? '#6366f1' : '#e2e8f050' }}>
            <Grid3X3 className="w-4 h-4" />
          </button>
          <button onClick={() => setView('list')} className="p-2.5" style={{ background: view === 'list' ? '#6366f110' : 'transparent', color: view === 'list' ? '#6366f1' : '#e2e8f050' }}>
      ...
      {showFilters && (
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            style={{
              borderColor: category === 'all' ? '#6366f1' : '#6366f140',
              background: category === 'all' ? '#6366f110' : 'transparent',
              color: category === 'all' ? '#6366f1' : '#e2e8f070',
            }}
          >
            Все
          </button>
          {allCategories.map(c => (
            <button
              ...
              className="px-3 py-1.5 rounded-full text-xs font-medium border transition-all"
              style={{
                borderColor: category === c ? '#6366f1' : '#6366f140',
                background: category === c ? '#6366f110' : 'transparent',
                color: category === c ? '#6366f1' : '#e2e8f070',
              }}
            >
              {c}
            </button>
        ...
        {filtered.map(product => (
          ...
          >
            ...
            <div className="p-4 flex-1">
              <h3 className="text-sm font-semibold line-clamp-2" style={{ color: '#e2e8f0' }}>{product.name}</h3>
              {product.description && view === 'list' && (
                <p className="text-xs mt-1 line-clamp-2" style={{ color: '#e2e8f050' }}>{product.description}</p>
              )}
              {product.rating !== undefined && (
                <div className="flex items-center gap-1 mt-1">
                  <Star className="w-3.5 h-3.5 fill-current" style={{ color: '#eab308' }} />
                  <span className="text-xs font-medium" style={{ color: '#e2e8f070' }}>{product.rating}</span>
                </div>
              )}
              <div className="flex items-center justify-between mt-3">
                <div className="flex items-baseline gap-2">
                  ...
                  {product.oldPrice && (
                    <span className="text-xs line-through" style={{ color: '#e2e8f050' }}>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12">
          <p className="text-sm" style={{ color: '#e2e8f050' }}>Товары не найдены</p>
        </div>
      )}
    </div>
  );
