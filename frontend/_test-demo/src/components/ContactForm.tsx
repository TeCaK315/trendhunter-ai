  ...
  if (sent) {
    return (
      <div className="rounded-2xl border p-8 text-center" style={{ borderColor: '#22c55e' }}>
        ...
        <h3 className="text-xl font-bold mb-2" style={{ color: '#e2e8f0' }}>Сообщение отправлено!</h3>
        <p className="text-sm" style={{ color: '#e2e8f070' }}>Мы свяжемся с вами в ближайшее время.</p>
        <button onClick={() => { setSent(false); setName(''); setEmail(''); setSubject(''); setMessage(''); }}
          className="mt-4 px-4 py-2 rounded-xl border text-sm" style={{ borderColor: '#6366f140', color: '#e2e8f070' }}>
          Отправить ещё
        </button>
      </div>
    );
  ...
  return (
    <div className="rounded-2xl border p-6" style={{ borderColor: '#6366f140' }}>
      <div className="mb-6">
        ...
        {subtitle && <p className="text-sm mt-1" style={{ color: '#e2e8f050' }}>{subtitle}</p>}
      </div>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-medium mb-1.5 flex items-center gap-1" style={{ color: '#e2e8f070' }}>
        ...
        <div>
          <label className="text-xs font-medium mb-1.5 block" style={{ color: '#e2e8f070' }}>Тема</label>
